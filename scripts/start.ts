...
const main = async () => {
  ...
  try {
    ...
    if (!search.list || !search.list.length) {
      search.list = [
        {
          name: '站内',
          icon: 'https://qiniu.image.tuotuoquan.com/sun_code/20250421173213-oBEnRe-logo.png',
          placeholder: '站内搜索',
          blocked: false,
          isInner: true,
        },
...

  {
    ...
    settings.favicon ??=
      'https://qiniu.image.tuotuoquan.com/sun_code/20250421173213-oBEnRe-logo.png'
...
`.trim()
    ...
    const defImgs = [
      {
        src: 'https://qiniu.image.tuotuoquan.com/sun_code/20250421172907-HZTyUR-IMG_6207.PNG',
        url: '',
      },
    ]
